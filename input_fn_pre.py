...
def random_adjust(image_T1, image_T2, label):
	"""Normalize data."""

	# Gamma校正
	# image_T1 = tf.image.adjust_gamma(image_T1, gain=1.0, gamma=1.4)
	image_T2 = tf.image.adjust_gamma(image_T2, gain=1.0, gamma=1.8)

	# 随机调整亮度
	# image_T1 = tf.image.random_brightness(image_T1, max_delta=0.5)
	image_T2 = tf.image.adjust_brightness(image_T2, -0.1)

	# 随机调整对比度
	image_T1 = tf.image.random_contrast(image_T1, lower=4.5, upper=5)
	image_T2 = tf.image.random_contrast(image_T2, lower=1.2, upper=1.5)



	return image_T1, image_T2, label

def val_test_adjust(image_T1, image_T2, label):
	"""Normalize data."""
	...
	# 调整对比度
	image_T1 = tf.image.adjust_contrast(image_T1, 4.7)
	image_T2 = tf.image.adjust_contrast(image_T2, 1.35)
...
def normalize_image(image_T1, image_T2, label):
	...
	# #Gamma校正
	# image_T1 = tf.image.adjust_gamma(image_T1,  gain=1.0, gamma=1.4)
	# image_T2 = tf.image.adjust_gamma(image_T2,  gain=1.0, gamma=1.4)
    #
	# #随机调整亮度
	# image_T1 = tf.image.random_brightness(image_T1, max_delta=0.5)
	# image_T2 = tf.image.random_brightness(image_T2, max_delta=0.5)
    #
	# #随机调整对比度
	# image_T1 = tf.image.random_contrast(image_T1, lower=0.8, upper=1.2)
	# image_T2 = tf.image.random_contrast(image_T2, lower=0.8, upper=1.2)
    #
...
def input_function(data_dir, mode, patch_size, batch_size, buffer_size, valid_id,
	...
	with tf.name_scope('input'):
		...
		if mode == 'train':
			...
			dataset = dataset.map(random_adjust, num_parallel_calls=num_parallel_calls)

		elif mode == 'valid':
			dataset = dataset.map(decode_valid, num_parallel_calls=num_parallel_calls)
			dataset = dataset.map(val_test_adjust, num_parallel_calls=num_parallel_calls)
		elif mode == 'pred':
